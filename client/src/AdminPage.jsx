...
const Admin_Page = () => {
  ...
  const [userEnvPaths, setUserEnvPaths] = useState([]); // ✅ new state
  const [showEnvPaths, setShowEnvPaths] = useState(false); // ✅ toggle
  ...
  const handleUserClick = async (username) => {
    ...
    setShowEnvPaths(false); // default to reports
    setUserReports([]);
    setUserEnvPaths([]);
    setError('');
    setLoading(true);
    ...
    try {
      const response = await axios.get("http://localhost:5000/get-env-paths", {
        headers: {
          ...
          "x-username": selectedUser, // Username header for admin querying others
  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (response.status === 200) {
        setUserReports(prev => prev.filter(item => item._id !== id && item.report_id !== id));
